...
import { CLEAR_ERROR_GET_ALL_REPORTS_VIDEO } from "@/constants/redux.contants";
import { useAppDispatch, useAppSelector } from "@/hooks";
import LayoutAdmin from "@/layouts";
import { parseTime } from "@/libs/helper";
import { getAllReportVideosStream } from "@/redux/actions";
...
const ReportManagePage = () => {
  ...
  const { loading, error, reportVideos } = useAppSelector(
    (state) => state.getAllReportVideoReducer
  );
  ...
  useEffect(() => {
    if (error) {
      ...
      dispatch({ type: CLEAR_ERROR_GET_ALL_REPORTS_VIDEO });
      router.push("/");
    }
  }, [error]);
  ...
  useEffect(() => {
    // @ts-ignore
    dispatch(getAllReportVideosStream());
  }, []);
        ...
        <LayoutAdmin>
          <div className='my-5 mx-5'>
            <h1 className='text-3xl font-extrabold text-gray-700 mb-4'>
              Manage reports video stream page
            </h1>
            ...
            <p className='text-base text-gray-400 mb-2 mt-2'>
              Tổng số report: {reportVideos?.length}
            </p>
                    ...
                    <table className='min-w-full '>
                      ...
                      <tbody>
                        {reportVideos?.length > 0 ? (
                          reportVideos.map((item: any, index: number) => (

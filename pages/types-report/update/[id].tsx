...
const UpdateTypeReport = () => {
  ...
  const { error, success } = useAppSelector((state) => state.createReportType);

  const [title, setTitle] = useState<string>("");
  const [description, setDescription] = useState<string>("");
  const [range, setRange] = useState<number>(1);

  const handleSubmitCreateNewTypeReportStream = async (
    e: FormEvent<HTMLFormElement>
  ) => {
    const id = toast.loading("Create post....");
    e.preventDefault();
    // @ts-ignore
    await dispatch(createNewReportType({ title, description, range }));
    toast.remove(id);
  };
  ...
  useEffect(() => {
    if (success) {
      ...
      dispatch({ type: RESET_CREATE_REPORT_TYPE, payload: undefined });
      router.push("/types-report");
    }

    if (error) {
      toast.error(error);
      dispatch({ type: CLEAR_ERROR_REPORT_TYPE, payload: undefined });
    }
  }, [success, error]);
  return (
    <LayoutAdmin>

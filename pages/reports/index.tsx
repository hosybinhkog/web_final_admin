...
import { CLEAR_GET_ALL_REPORT_POST_ERROR } from "@/constants/redux.contants";
...
const ReportManagePage = () => {
  ...
  const { loading, error, reportPosts, reportPostsCount } = useAppSelector(
    (state) => state.getAllReportPostReducer
  );
  ...
  useEffect(() => {
    if (error) {
      ...
      dispatch({ type: CLEAR_GET_ALL_REPORT_POST_ERROR });
      router.push("/");
    }
  }, [error]);
      ...
      {loading ? (
        ...
        <LayoutAdmin>
          <div className='my-5 mx-5'>
            ...
            <p className='text-base text-gray-400 mb-2 mt-2'>Tổng số report: {reportPostsCount}</p>
                    ...
                    <table className='min-w-full '>
                      ...
                      <tbody>
                        {reportPosts?.length > 0 ? (
                          reportPosts.map((item: any, index: number) => (

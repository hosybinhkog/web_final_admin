...
import { getAllReportTypesVideosStream } from "@/redux/actions";
import { useRouter } from "next/router";
import { useEffect } from "react";
import toast from "react-hot-toast";
import { Loading, Seo } from "../../components";
import { CLEAR_ERROR_GET_ALL_REPORTS_TYPE_VIDEO } from "../../constants/redux.contants";
import { useAppDispatch, useAppSelector } from "../../hooks";
import LayoutAdmin from "../../layouts";

const ReportVideoCategoriesPage = () => {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { loading, error, reportTypesVideos } = useAppSelector(
    (state) => state.getAllReportTypeVideoReducer
  );

  useEffect(() => {
    dispatch(getAllReportTypesVideosStream());
  }, []);

  useEffect(() => {
    if (error) {
      toast.error(error);
      dispatch({ type: CLEAR_ERROR_GET_ALL_REPORTS_TYPE_VIDEO });
      router.push("/");
    }
  }, [error]);
      ...
      {loading ? (
        ...
        <div className='my-5 mx-5'>
          <h1 className='text-3xl font-extrabold text-gray-700 mb-4'>
            Manage types - report - video - stream
          </h1>
          ...
          <p className='text-base text-blue-500 mb-2 mt-2 font-extrabold'>
            <span className='font-bold text-gray-900'>
              Count types - report - video - stream : {` `}
            </span>
            {reportTypesVideos?.length}
          </p>
                          ...
                          .map((item: any, index: number) => (
                            <tr key={item._id} className='border-b rounded-md'>
                              ...
                              <td className='text-md text-gray-900 font-semibold px-2 py-1 whitespace-nowrap'>
                                ...
                                <button
                                  onClick={() =>
                                    router.push(`/reports-videos-cate/update/${item?._id}`)
                                  }
                                  type='button'
                                ...
                                <button
                                  onClick={() => router.push(`/reports-videos-cate/${item?._id}`)}
                                  type='button'
                                  className='text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-md px-5 py-2.5 mr-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700'
                                >
                                  views
          ...
          <button
            onClick={() => router.push("/reports-videos-cate/new")}
...
export default ReportVideoCategoriesPage;
